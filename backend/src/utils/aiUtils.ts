...
interface KeywordType {
  word: string;
  importance: number; // 1-10 scale for word cloud sizing
}
 ...
 */
export const generateFlashcards = async (content: string): Promise<FlashcardType[]> => {
  try {
    const prompt = `
 ...
 */
export const extractKeywords = async (content: string): Promise<KeywordType[]> => {
  try {
    const prompt = `
