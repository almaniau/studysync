...
const initializeSocket = (server) => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('joinStudyGuide', (studyGuideId) => {
    ...
    socket.on('leaveStudyGuide', (studyGuideId) => {
  ...
  const emitStudyGuideCreated = (studyGuide) => {
    io.emit('studyGuide:created', studyGuide);
  };

  const emitStudyGuideUpdated = (studyGuideId, updatedBy, updatedAt) => {
  ...
  const emitStudyGuideDeleted = (studyGuideId) => {
    io.to(`studyGuide:${studyGuideId}`).emit('studyGuide:deleted', {
      studyGuideId
    });
  };

  const emitStudyGuideUpvoted = (studyGuideId, upvotes, upvotedBy) => {
...
module.exports = { initializeSocket, socketEvents };
